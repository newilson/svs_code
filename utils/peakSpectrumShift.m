function [spec_sh,hzshift] = peakSpectrumShift(spec,hz,f0,lb,peaks,ranges)
%
% 
...
specmag = abs(fftshift(fft(fid,[],1),1));
...
for ii=1:size(specmag,2)
    ...
    for jj=1:npeaks
        ...
        [~,zpind] = max(col(specmag(ind,ii)));
        zpsh(jj) = ppm0(ind(1)-1+zpind) - peaks(jj);
    end
    zp = mean(zpsh);
    hzshift(ii) = zp*f0;
