...
%% head coil data
mainDir = 'Z:\Data\Spectroscopy\Brain 7T - 31P\U01\test';
scanDir = 'S092_03052026_31P-1H-HeadCoil';

datfiles = dir([mainDir filesep scanDir filesep 'datfiles' filesep '*31p*.dat']);
...
%     With >=2 peaks, dat2mat_svs31p fits a linear phase ramp through them
%     and applies it (zero-order + first-order in one shot). PCr / gATP /
%     aATP are the most reliable, well-isolated 31P landmarks at 7T.
pars.peaks      = [0, -2.53, -7.56];   % PCr, gATP, aATP (ppm)
pars.peakRanges = [0.5 0.5;            % PCr   ±0.5 ppm
                   0.4 0.4;            % gATP  ±0.4 ppm
                   0.3 0.3];           % aATP  ±0.3 ppm (avoid NADH at -8.11)
...
pars.plt              = true;
pars.block_align.size = []; % no block averaging
...
shB = repmat([-60 60], nMet, 1);                          % default
...
shB(strcmp(pars.fit.metabs,'Broad10_5'),:) = [-300 60]; % keep wide
...
for ii=5%1:length(datfiles)
    ...
    [tempout,pars] = dat2mat_svs31p(pars,curfile);
end
