function [ws, nws, ppm] = dat2mat_svsMEGA(ws_dat,nws_dat)
%
...
nws = squeeze(sum(nws,3)); % edit dimension does not effect water signal
...
% take difference and store as 3rd Ide
% ws(:,:,3) = ws(:,:,1) - ws(:,:,2);
...
if plt
    NWplayplot(real(fftshift(fft(ws,[],1),1)),ppm);
...
peaks = [3.03 3.21 2.02]; % cr-cho-naa
ranges = 0.1*ones(length(peaks),2);
addlb = 4; % additional line broadening to locate peaks
[~,hzshift] = peakSpectrumShift(ws(:,2),hz,f0,addlb,peaks,ranges);
ws = shiftSpectrumFrequency(ws,hzshift,t);
if plt
    NWplayplot(ws1,ppm,[],ws,ppm);
end
% peaks = peaks(1); % phase creatine only
% ranges = ranges(1,:);
[~,ph] = peakSpectrumPhase(ws(:,2),ppm,t,addlb,peaks,ranges/2);
ws = shiftSpectrumPhase(ws,ph); % 0 order only
if plt
    NWplayplot(real(ws),ppm);
%     plot(ppm,real(sum(ws,2)),'r')
%     figure, plot(hzshift,'-*')
end

% difference editing
disp('difference editing')
ws(:,3) = ws(:,1)-ws(:,2); 

% disp('resolve block averages')
% ws = mean(ws,2);

% disp('baseline correction') % baseline correction not needed bc of subtraction
